...
`waveform` は配列位置や数値IDをJSONに直接記録せず、Wave table定義側の名前を参照する。hUGETrackerの `TInstrumentV3.Waveform` および既存のVersion 6生成処理では内部的に0始まりの数値を使用するため、変換時には名前をWave table側で定義された数値へ解決する。名前から数値への対応、数値IDの範囲、Wave tableの最大定義数は後続WBS「CH3 / Wave table定義項目を決める」で確定する。
