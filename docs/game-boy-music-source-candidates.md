...
### objetdiscret 7曲の用途・曲調（試聴前の配布元情報による暫定整理、2026-08-01）

公式ページはこのパックをGBゲーム用に作成したサウンドトラックと説明し、`.uge`をGB Studio/hUGETracker対応ファイルとして列挙している。また、Track Listで各曲の原曲と作曲家を示している。この節は、公式配布ページから確認できる曲名、ファイル名の用途語、原曲情報を試聴前の暫定情報として保存したものとする。人による試聴後の曲単位整理は後述の「A曲21曲の配布元情報と人による試聴結果の曲単位整理」を正本とする。
...
| 用途 | 暫定候補 | 状態 |
|---|---|---|
| タイトル | 該当候補なし | 試聴結果ではPocket Sweeper用途に不向き |
| メニュー | 該当候補なし | 試聴結果ではPocket Sweeper用途に不向き |
| 通常プレイ | 該当候補なし | 試聴結果ではPocket Sweeper用途に不向き |
| ポーズ | 該当候補なし | 試聴結果ではPocket Sweeper用途に不向き |
| クリア | 該当候補なし | 試聴結果ではPocket Sweeper用途に不向き |
| エンディング | Overworld-Liebestraum、Gallery-Pathetique、Underwater-Tristesse | 人の試聴による用途候補。ただし最終採用なし |
| ゲームオーバー | 該当候補なし | 試聴結果で該当なし |
| 短い演出 | 該当候補なし | 試聴結果で該当なし。技術的な切り出し可否は未確認 |
| その他 | Gallery-Pathetiqueの記録画面候補 | Pocket Sweeper固有用途としては未確定 |
| 未確定 | 7曲の採用可否、テンポ、音量、技術的なループ構造 | 人の試聴済み。採用なし。技術的事項は構造解析の記録を参照 |

試聴時は、タイトル・通常プレイ・ポーズ・クリア・エンディングへの適性、思考を妨げないか、明るさ・穏やかさ・緊張感、ループ境界、1ループの長さ、音量感、CH4/Noiseの目立ち方、既存SFXとの干渉を確認する。2026-09-25に人が7曲を試聴し、用途候補・曲調・長時間BGM適性・ループ時の違和感を確認した。表の暫定候補はこの確認結果に置き換えられ、7曲からPocket Sweeperへ採用する曲はない。技術的な音源構成は試聴結果から断定しない。

### A曲21曲の人による試聴結果（2026-09-25）

WBS-001-01545の証跡として、人がA判定済み21曲（Yogi #1 4曲、Yogi #2 10曲、objetdiscret 7曲）を実際に試聴した。全曲について用途候補・曲調・長時間BGM適性・ループ時の違和感を評価し、個別所見を `wbs/tasks/WBS-001-01545.md` に記録した。Yogiは雰囲気変化や暗さにより用途が限定的で、objetdiscretは音色・アレンジが単調に感じられた。一部にエンディング候補はあるが、21曲全体のPocket Sweeperへの最終採用はなしと判断した。なお「矩形波しか使っていない」は試聴結果としては記録しない。
...
対応集計は、確定18曲、暫定0曲、不一致0曲、確認不能0曲である。実ファイルはPack #1 UGE 8件、Pack #2 UGE 10件の計18件、ASM 0件、INC 0件で、空ファイル・同一内容ファイル・ファイル名重複は確認されなかった。配布ページ記載曲数（8曲、10曲）とも一致する。A曲21曲のUGE内部曲名、Song Version・CH1〜CH4・CH4/Noise・loop構造は後述の構造解析で確認し、人による試聴結果と配布元情報を組み合わせた曲単位整理は後述の整理節および [WBS-001-01546](../wbs/tasks/WBS-001-01546.md) に記録する。

### A曲21曲の配布元情報と人による試聴結果の曲単位整理（2026-09-25）

対象はA判定済み21曲（Yogi #1 4曲、Yogi #2 10曲、objetdiscret 7曲）である。配布元・UGEファイル名・A区分・由来情報は本節以前の公開一次情報および実ファイル確認を根拠とし、用途候補・曲調・長時間BGM適性・ループ聴感・個別所見は [WBS-001-01545](../wbs/tasks/WBS-001-01545.md) の人による試聴結果を根拠とする。曲単位の全項目をまとめた正本表は [WBS-001-01546](../wbs/tasks/WBS-001-01546.md) に置き、配布元情報と人の評価を混同しない。

人の整理では、Delight、Overworld-Liebestraum、Gallery-Pathetique、Underwater-Tristesseの4曲をエンディング用途候補として残す。ただし4曲ともPocket Sweeperへの最終採用はなしである。その他17曲の用途候補は不向きであり、21曲すべての最終採用はなしとする。これは人による判断であり、Codexの試聴・音楽的評価ではない。既存のUGE構造解析（Song Version、CH1〜CH4、CH4/Noise、order、pattern、技術的loop）は別情報として保持し、人のループ聴感・曲調評価を上書きしない。
...
### WBS-001-01547 再集計（2026-09-25）

既存情報を母集団ごとに再集計した。用途・曲調は、WBS-001-01545/01546で人が試聴・整理したA判定済み21曲のみを分母とし、Codexによる試聴・音楽的評価は行っていない。作者・配布元・ライセンス・形式は、確認範囲に応じて分母を分けた。

| 集計項目 | 分母 | 内訳 |
|---|---:|---|
| 作者（ページ上の曲数確認） | 36曲 | lillstrumpa 11、Yogi 18、objetdiscret 7 |
| 配布元／パック（ページ上の曲数確認） | 36曲 | lillstrumpa vol.1〜3 11、Yogi #1 8、Yogi #2 10、Fabulous Classical Chiptunes 7 |
| 作者（A判定） | 21曲 | Yogi（Tronimal）14、objetdiscret 7 |
| 配布元／パック（A判定） | 21曲 | Yogi #1 4、Yogi #2 10、Fabulous Classical Chiptunes 7 |
| ライセンス表示（ページ確認） | 36曲 | CC BY 4.0表示あり 36（100%） |
| プロジェクト受入判定（実ファイル詳細確認） | 25曲 | A 21、D 4、未判定 0（lillstrumpa 11は実ファイル未確認のため別母集団） |
| 配布ページ記載形式（ページ確認） | 36曲 | UGE 36、ASM 0。WAV併記 11、FLAC併記 7 |
| 実ファイル確認形式（詳細確認） | 25曲 | UGE 25、ASM 0。A判定21曲の解析対象はUGE 21 |
| 用途候補（人による評価） | 21曲 | エンディング 4、 不向き 17 |
| 最終採用（人による判断） | 21曲 | 採用 0、採用なし 21 |

作者と配布元／パックは別概念として数えた。`Yogi（Tronimal）`は既存資料の作者表記、`Yogi #1/#2`は配布元／パック表記であり、新たな名寄せはしていない。ライセンスは公開ページ表示、同梱記載、第三者楽曲由来、A〜D受入判定を同一概念に単純化していない。CC BY 4.0表示があることは、各曲の既存の条件・第三者権利に関する留保を取り除くものではない。

#### A判定21曲の曲調タグ（人による評価）

WBS-001-01546の21行の「人による曲調」をタグ単位で再集計した。
...
曲調は複数タグを許可するため、タグ件数の合計は対象曲数（21曲）と一致しない場合がある。たとえば「暗い、軽快」は暗い1件・軽快1件として数え、新しい複合カテゴリは作成していない。A判定21曲の内訳はYogi #1 4曲、Yogi #2 10曲、objetdiscret 7曲であり、`4 + 10 + 7 = 21`。用途も`4 + 17 = 21`である。D判定4曲、未判定11曲、配布ページのみで確認した候補は、用途・曲調の21曲集計へ混在させていない。なお、本節では01548の不足分析・提案を実施しない。

### WBS-001-01548 不足状況の整理（2026-09-25）

01547の再集計、01545/01546の人による試聴・採用判断、および本書の既存調査結果を母集団として、現在の不足・未確保・未確認を整理した。候補数と最終採用数は別に扱い、未判定曲や保留候補を採用済みへ繰り上げていない。

| 観点 | 現在の実数 | 確認状態 | 不足・未確保の内容 | 根拠 | 次に確認すべき条件 |
|---|---:|---|---|---|---|
| タイトルBGM | 最終採用0曲（A判定21曲中、用途候補0曲） | 仕様上の用途あり。人の試聴では採用なし | タイトル用途の採用曲を未確保。必要曲数は仕様に明記なし | `docs/sound-spec.md` のタイトルBGM方針、01545/01546 | タイトル画面に適合し、長時間再生・ループを人が確認して採用判断する |
| 通常プレイBGM | 最終採用0曲（用途候補0曲） | 仕様上の用途あり。人の試聴では採用なし | 通常プレイ用途の採用曲を未確保。必要曲数は仕様に明記なし | `docs/sound-spec.md` のプレイ中BGM方針、01545/01546 | 長時間聴いても妨げにくく、ループを人が確認して採用判断する |
| ポーズBGM | 最終採用0曲（用途候補0曲） | 仕様上のBGM用途として必要曲数は未確定。試聴結果では該当採用なし | ポーズ用途の採用曲を未確保 | `docs/sound-spec.md` のサウンド用途、01545/01546 | ポーズ画面での使用要否と適合を仕様・人の試聴で確認する |
| クリアBGM | 最終採用0曲（エンディング候補4曲を含む） | 仕様上の用途あり。エンディング候補4曲は採用なし | クリア用途の採用曲を未確保。必要曲数は仕様に明記なし | `docs/sound-spec.md` のクリアBGM方針、01545/01546 | BGMとして使うか短い効果音とするかを含め、人が採用判断する |
| ゲームオーバーBGM | 最終採用0曲（用途候補0曲） | 既存仕様ではゲームオーバーは効果音用途として記載。BGM必要数は未確定 | ゲームオーバーBGMの採用曲は未確保。ただしBGM曲数不足とは断定しない | `docs/sound-spec.md` のゲームオーバー効果音、01545/01546 | BGMを必要とするか、効果音で扱うかを仕様で確認する |
| 曲調 | 軽快10、暗い9、緊張感7、穏やか5、幻想的3、哀愁1（35タグ） | A判定21曲の人による複数タグ | タグは存在するが、21曲すべて最終不採用。Pocket Sweeper用途に適した採用曲が未確保。Yogi系の曲中変化、暗め・RPG向け評価、objetdiscretの単調さは既存評価のまま | 01545/01546/01547 | 用途適合、長時間再生、曲中変化、ループを人が確認する。タグ数だけで十分・不足を決めない |
| 作者 | A判定21曲をYogi（Tronimal）14曲、objetdiscret 7曲の2作者 | A判定・詳細確認・試聴済み母集団 | A判定素材が2作者に集中し、21曲すべて最終不採用。既存2作者以外を調査する余地がある。lillstrumpaは11曲あるが未判定 | 01547、候補調査本文 | 作者、曲単位情報、実ファイル、ライセンス条件を確認する。必要作者数は設定しない |
| ASM | 詳細確認済みASM 0曲、UGE 25曲 | 実ファイル詳細確認済み25曲 | ASM形式の詳細確認済み・採用可能曲は未確保。hUGEDriver `rgbds_example/sample_song.asm` は保留1件だが、曲名・作者・用途の曲単位情報不足で未確定 | 01547、本書のSuperDisk候補記録 | 曲単位の作者・用途・ライセンス・解析可能性を確認する。保留1件をA判定・採用候補へ昇格しない |

母集団の内訳は、配布ページ上36曲（lillstrumpa 11、Yogi 18、objetdiscret 7）、実ファイル詳細確認25曲（UGE 25、ASM 0）、A判定21曲（Yogi #1 4、Yogi #2 10、objetdiscret 7）である。エンディング用途候補4曲と不向き17曲は21曲を構成するが、最終採用は0曲、採用なしは21曲である。曲調タグは複数付与のため35タグであり、21曲とは一致しない。

lillstrumpa vol.1〜3の11曲は、配布ページ上の曲数・UGE/WAV・CC BY 4.0表示まで確認済みだが、実ファイル、曲単位情報、A〜D判定、人による試聴は未確認である。この11曲をA判定21曲の不足を埋める確定候補として数えない。一方、次の調査対象となり得る未確認の既存候補として11曲を記録する。

今後の候補は、仕様上必要な用途への適合、長時間BGMとしての人による試聴、曲中変化とループの確認、商用利用・改変・再配布条件の確認、UGEまたはASMとしての解析可能性を満たすことを確認する。既存2作者以外も調査対象となり得る。ASM候補は曲単位の情報（曲名、作者、用途、ライセンス、解析可否）を確認する。これらは条件整理であり、新しい候補の選定・取得・ライセンス判定ではない。

本節では、01549の「利用可能な約20曲」の最終確定、lillstrumpaのA判定、新規素材の検索・取得、人による採用判断を実施していない。

### 用途別・曲調別

試聴結果はA曲21曲について確認済みだが、ページ確認36曲全体の用途別・曲調別集計は未実施である。以下は未試聴の候補を含む配布元説明による暫定タグであり、今回の21曲の個別評価とは分けて扱う。
...
| 曲調不明 | 15 | 36 | A曲21曲は人の試聴で確認済み。lillstrumpa等未試聴候補を含む |
...
- 未試聴候補の用途・曲調、CH4/Noise使用傾向、4チャンネルの多様性。
