...
### objetdiscret 7曲の用途・曲調（試聴結果反映、2026-09-25）

公式ページはこのパックをGBゲーム用に作成したサウンドトラックと説明し、`.uge`をGB Studio/hUGETracker対応ファイルとして列挙している。また、Track Listで各曲の原曲と作曲家を示している。[公式配布ページ](https://objetdiscret.itch.io/fab-class-tunes)から直接確認できる曲名、ファイル名の用途語、原曲情報を「確定情報」とした。原曲の一般的な音楽的性格とは分けて、objetdiscretによるUGE編曲の聴感を人が確認した結果を記録する。Pocket Sweeperでの用途は試聴による候補評価であり、採用を確定するものではない。
