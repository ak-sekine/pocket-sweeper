...
- ローカルの `twinkle.uge` には `0x5A` が多数含まれており、hUGETracker / hUGEDriver側の休符またはNo Note値である90と対応する可能性が高い。ただし、変換ツール実装では公式ソース上の定数・挙動を優先して確認する。
...
- `.uge` バイナリを書き出す。
- Song Versionは、実装時に使用するhUGETracker実行版で開ける形式を確認して決める。候補は、ローカルサンプルで確認したVersion 6、または現行ソースのVersion 7とする。
- `title` を曲名として保存する。
...
- Song Version 6と7で `TicksPerRow` の構造が異なるため、対象Versionを固定してから実装する。
...
- 初版ツールでSong Version 6と7のどちらを書き出すかを、使用するhUGETracker実行版で確認する。
- note番号、休符値、effect code、effect paramの正確な対応を公式ソースまたはExport ASMと照合する。
...
- [ ] サウンド
  ...
  - [x] 楽曲定義JSON方式を決める
    ...
    - [x] `.uge` 生成物の扱いを決める
    - [x] `.uge` 形式の調査を行う
  - [ ] hUGETrackerで使用するSong Version（6/7）を決定する
  - [ ] note番号・休符値の対応を確認する
  - [ ] effect code / effect parameterの対応を確認する
