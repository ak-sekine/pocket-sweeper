...
- `title`: 曲名または効果音名。
- `type`: `bgm` または `sfx`。
- `tempo`: 曲テンポ。hUGETracker側の値へどう対応させるかは `.uge` 形式調査時に確認する。
- `instruments`: 音色定義。初版では音色番号と簡単な名前・種別を中心に扱う。
- `order`: 再生するパターン順。
- `patterns`: パターン定義。
- `channels`: `pulse1`、`pulse2`、`wave`、`noise` の4チャンネル。
- 各ノート情報:
  - `note`: 音名。休符は `rest` とする。
  - `length`: 音の長さ。初版では行数またはステップ数として扱い、`.uge` 変換時の具体的な解釈はツール実装時に決める。
  - `instrument`: 使用する音色番号。
...
| 詳細音色パラメータ | `Duty`、`InitialVolume`、`OutputLevel` など | 初版JSONでは持たない。hUGETracker初期値またはデフォルトInstrument相当を使う。 |

- 初版では、JSONの `id`、`name`、`channel` だけでInstrumentを生成する。
...
- 初版では詳細パラメータをJSONへ追加しない。音色の細かい調整は生成後にhUGETrackerで行う。
...
- Duty 1～8は `LoadDefaultInstruments` 相当の名前とduty比を使う。JSONで `name` が指定されたInstrumentだけ名前を上書きする。
- Wave/Noiseは初版では詳細編集を行わず、hUGETracker初期値相当の固定値で生成する。
- `SubpatternEnabled` は全Instrumentで `False` とし、`Subpattern` は空パターンにする。
- JSONで詳細音色パラメータを持たないため、最初の下書き生成後にhUGETrackerで音色を確認・微調整する。
...
- wave/noiseを初版で空扱いにした場合、hUGETracker上で警告なく開けるか確認する。
- 生成 `.uge` をhUGETrackerで開き、再生、保存、ASM Exportできることを確認する。
...
- [ ] サウンド
  ...
  - [ ] wave/noise未使用時でも正常に開けることを確認する
  - [ ] 生成した `.uge` をhUGETrackerで読み込み・保存・ASM Exportできることを確認する
