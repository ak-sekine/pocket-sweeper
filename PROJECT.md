...
- 効果音実装方式は、実装前に調査・比較して決定する。
- APUレジスタを短時間だけ直接書き換える軽量SFX方式は採用候補だが、現時点では正式決定ではない。
- hUGEDriverのみでBGMと効果音を共存させる方式、APUレジスタ直接制御方式、両方式の組み合わせを比較し、Pocket Sweeper初版に最も適した方式を選ぶ。
- 既存のJSON→hUGEDriver用ASM生成フローを可能な限り本番ROMでも活用できる実装方式を優先的に検討する。
- 効果音更新を `hUGE_dosound` の後に行い、効果音が使うチャンネルだけをそのフレームで短時間上書きする方式は、APU直接制御を採用する場合の候補とする。
- UI効果音は短いため、BGMの該当チャンネルが一瞬上書きされることは許容する。
- BGM制作では、初版は可能な範囲でPulse2とWaveを中心に使い、Pulse1はUI効果音用に上書きされても破綻しにくい使い方にする。
- 効果音制作で作成したJSONおよびhUGEDriver ASMは、単体確認用テストROMでの確認に使う。本体組み込み時の扱いは、効果音実装方式の調査・決定後に確定する。
...
- [ ] サウンド
  ...
  - [ ] サウンド実装
    ...
    - [ ] 効果音実装方式を調査・決定する
      - [ ] hUGEDriverのみでBGMと効果音を共存させる方法を確認する
      - [ ] APUレジスタ直接制御で効果音を鳴らす方法を確認する
      - [ ] 両方式のメリット・デメリットを比較する
      - [ ] 現在のJSON→ASM生成フローを本番でも活用できるか確認する
      - [ ] Pocket Sweeper初版に最も適した実装方式を決定する
