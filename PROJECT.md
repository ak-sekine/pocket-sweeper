...
- [ ] サウンド
        ...
        - [x] Version 2のloop動作をSameBoyまたはBGBで手動確認する
          ...
          - [x] BGM終了後もSFXのmute・再生経路を利用できることを確認する
            - [x] Codexによる実装・自動確認
              ...
              - [x] SFX開始時に対象チャンネルをmuteすることを確認する
              - [x] SFX再生中にAPU直接制御の更新処理が継続することを確認する
  ...
  - [ ] BGM制作
    ...
    - [x] ChatGPTまたはCodexによるVersion 2楽曲定義JSONの直接制作運用を検証する（初版BGM制作では不採用）
      ...
      - [x] 再修正した運用確認用JSONからASMと確認用ROMを再生成する
        - [x] 再修正後の運用確認用JSONに対するCodexのASM/ROM再生成・自動確認
        - [x] 再修正後の運用確認用ROMを人がSameBoy確認する
          - [x] 軽快さ・単調さ・Noiseの再修正効果未達を記録する
          - [x] 既存曲の部分修正をやめ、曲構成から再設計する方針を決める
      - [x] ChatGPTでMIDIを試作し、採用MIDIをCodexでVersion 2 JSONへ変換する運用を検証する（初版BGM制作では不採用）
        - [x] ChatGPTで運用確認用BGMのMIDI試作条件を整理する
        ...
        - [x] 人がMIDI原曲とSameBoy上のGB版を比較試聴する
          - [x] MIDI原曲とGB版が大きく異なり、CH1主旋律が曲として成立していないことを記録する
          - [x] tempo 6の速度感は問題ないことを記録する
          - [x] CH4 Noiseが曲に合わず雑音として聞こえることを記録する
          - [x] CH2 supportが曲に合わず不協和音として聞こえることを記録する
        - [x] MIDI承認後のGB編曲・JSON変換結果に問題があれば、その範囲を調整する（検証完了。不採用）
          - [x] 人手試聴結果を基にGB編曲・JSONを修正する
          - [x] 修正版ASM・ROMを再生成して自動確認する
          - [x] 人が修正版GB版をSameBoyで再試聴する
            - [x] 修正版でもちゃんとしたBGMとして成立していないことを記録する
            - [x] AIによるGB編曲修正を継続しても初版制作フローとして有効とは判断できないことを記録する
      - [x] AI作曲・MIDI変換方式の検証結果を整理する
        - [x] ChatGPT / Codexによる作曲を初版BGM制作では採用しないことを決める
    ...
    - [ ] ルールベースによるGame Boy BGM自動生成方式を検証する
      - [x] 自動作曲方式の目的・適用範囲・中止条件を整理する
        - [x] 自動作曲基盤の再利用範囲を決める
        - [x] MIDIを中間形式として使用せず、Game Boy向け楽曲データを直接生成する方針を決める
        - [x] AIによる音符の直接生成ではなく、明示的なルールと既存曲から整理した設計パターンで生成する方針を決める
        - [x] 生成結果の品質が実用水準に達しない場合の検証中止条件を決める

      - [ ] ルール抽出元として利用できるGame Boy楽曲データを整理する
        - [x] UGE / hUGEDriver ASMなど、構造を直接解析できる形式を対象とする
        - [x] 利用・解析・GitHub管理上のライセンス条件を確認する
        - [ ] 利用可能な楽曲数と用途・曲調の偏りを整理する
          - [x] [Codex] 配布ページ上の候補曲数とライセンス表示を整理する
          - [x] [Codex] 曲数確認済み・曲名確認済み・実ファイル未確認を分けて整理する
          ...
          - [x] [Codex] 曲単位でライセンス適用範囲とA〜D区分を確定する
            - [x] [Codex] 公開一次情報だけで判定する受入基準を決める
            - [x] [Codex] Pack #1の第三者楽曲由来4曲を採用対象外にする
          ...
          - [x] [Codex] 不足する6曲以上を明確な公開ライセンスのUGE／ASM素材から追加選定する
            - [x] [Codex] 公式配布ページとライセンスが明確な追加候補を探す
            - [x] [Codex] remix・cover・第三者楽曲由来の候補を除外する
            - [x] [人] objetdiscret「Fabulous Classical Chiptunes」を公式itch.ioページから取得し、`local/music-source-candidates/objetdiscret-fab-class-tunes/original/`へ配置する
            - [x] [Codex] 取得した追加素材の実ファイル数とライセンス適用範囲を確認する
            - [x] [Codex] 確認済み追加素材の用途・曲調を整理する
        - [ ] 20曲程度の少数データを統計学習用コーパスではなく設計パターン抽出元として扱う方針を決める

      - [ ] 既存Game Boy楽曲から抽出する設計情報を決める
        - [ ] 曲全体の構成、order、pattern、loop構造を整理する
        - [ ] CH1～CH4の役割と発音密度を整理する
        - [ ] 音域、音程遷移、note length、休符の使い方を整理する
        - [ ] motifの反復・変奏パターンを整理する
        - [ ] CH2の伴奏パターンを整理する
        - [ ] CH3のベースパターンを整理する
        - [ ] CH4のリズムパターンを整理する
        - [ ] Instrument、Wave table、Noise Instrument、effectの使用傾向を整理する

      - [ ] Game Boy楽曲解析ツールを試作する
        - [ ] UGEまたはASMから必要な情報を読み取れることを確認する
        - [ ] 解析結果を共通の中間表現へ変換する
        - [ ] 曲・pattern・channel単位の特徴を自動出力する
        - [ ] 少数の既存曲で解析結果を人が確認する

      - [ ] 自動作曲用のルール・パターン表現を決める
        - [ ] ハードウェア制約に由来するルールを整理する
        - [ ] 一般的な音楽理論に由来するルールを整理する
        - [ ] 既存Game Boy楽曲から確認できた設計パターンを整理する
        - [ ] 各ルールに根拠を記録できる形式を決める
        - [ ] melody motif、rhythm、harmony、bass、noiseのパターンを個別に管理する方法を決める
        - [ ] ルール不足時に根拠なく新規ルールを追加しない運用を決める

      - [ ] ルールベースBGM生成ツールを試作する
        - [ ] Pythonで再現可能なseed付き生成処理を実装する
        - [ ] 曲構成とloop構造を生成する
        - [ ] CH1主旋律をルール・motifパターンから生成する
        - [ ] CH2伴奏をルール・伴奏パターンから生成する
        - [ ] CH3ベースをルール・ベースパターンから生成する
        - [ ] CH4リズムをルール・Noiseパターンから生成する
        - [ ] Game Boy 4chとhUGEDriverの制約を生成時に保証する

      - [ ] 生成曲から確認用ROMまでを一括生成できるようにする
        - [ ] MIDIを経由せず生成結果をGame Boy向け楽曲データへ変換する
        - [ ] hUGEDriver用ASMを生成する
        - [ ] 確認用ROMを生成する
        - [ ] seedを指定して同一曲を再生成できるようにする
        - [ ] 1コマンドで楽曲生成から確認用ROM生成まで実行できるようにする

      - [ ] ルールベース自動作曲方式の有効性を評価する
        - [ ] 複数seedで候補曲を生成する
        - [ ] 人がSameBoyで候補曲を試聴する
        - [ ] 実用候補となるBGMが生成できるか確認する
        - [ ] 問題がある場合は既存曲との差を解析可能な特徴量で比較する
        - [ ] ルール追加・変更に明確な根拠を持てるか確認する
        - [ ] 今後のGame Boyゲーム開発で継続利用するか判断する
    ...
    - [ ] 初版用タイトルBGMのhUGETracker正本データ `assets/bgm_title.uge` を人が作成・確定する
    - [ ] 初版用プレイ中BGMのhUGETracker正本データ `assets/bgm_game.uge` を人が作成・確定する
    - [ ] 初版用クリアBGMのhUGETracker正本データ `assets/bgm_clear.uge` を人が作成・確定する
    - [ ] 初版用BGM3曲の音色・音量・チャンネル構成の統一性を人が確認する
    - [ ] 初版BGM3曲をゲームへ組み込み、SameBoyで最終確認する
  - [ ] 効果音制作
    - [x] 必要な効果音一覧を決定する
    - [x] カーソル移動効果音を作成する
    - [ ] 決定効果音を作成する
