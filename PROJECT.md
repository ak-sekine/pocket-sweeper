...
- 盤面表示、カーソル移動範囲、地雷配置範囲、数字生成範囲、クリア判定、BFSキュー、表示更新キューは後続WBSで可変サイズ対応する。
...
- BG表示更新をVBlankへ送るための表示更新キューと必要に応じた上書きタイル番号をWRAMに保持する。最大100セル対応は後続WBSで行う。
- 表示更新キューの上書きタイル番号は、0を自動計算、1以上を `tile + 1` として扱う。
- 0連鎖オープン用のBFSキュー領域をWRAMに保持する。最大100セル対応は後続WBSで行う。
