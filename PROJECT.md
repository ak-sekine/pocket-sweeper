  ...
  - [ ] BGM制作
    ...
    - [ ] ルールベースによるGame Boy BGM自動生成方式を検証する
      - [ ] 自動作曲方式の目的・適用範囲・中止条件を整理する
        - [ ] 今後のGame Boyゲーム開発で再利用できる共通作曲基盤を目的とする
        - [ ] MIDIを中間形式として使用せず、Game Boy向け楽曲データを直接生成する方針を決める
        - [ ] AIによる音符の直接生成ではなく、明示的なルールと既存曲から整理した設計パターンで生成する方針を決める
        - [ ] 生成結果の品質が実用水準に達しない場合の検証中止条件を決める

      - [ ] ルール抽出元として利用できるGame Boy楽曲データを整理する
        - [ ] UGE / hUGEDriver ASMなど、構造を直接解析できる形式を対象とする
        - [ ] 利用・解析・GitHub管理上のライセンス条件を確認する
