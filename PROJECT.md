...
- SFX開始時はchannel kindに応じて `hUGE_mute_channel` で対象チャンネルをミュートする。現行実装ではPulse1効果音はCH1、Noise効果音はCH4を使う。
...
- 現在の `assets/se_cursor.json` はPulse1を使用しているため、チャンネル運用方針に合わせてNoiseチャンネルへ変更する。
...
- 現在のJSONはPulse1を使い、高音 `C8`、`tempo = 1`、短い減衰のPulse Instrumentで構成している。
- Instrument詳細は `duty = 0`、`initial_volume = 3`、`envelope_direction = "down"`、`envelope_sweep = 1` とし、余韻を抑えた軽い「ピッ」という音を狙う。
- チャンネル運用方針ではカーソル移動のような音程を必要としないUI効果音はCH4 / Noiseを優先するため、`assets/se_cursor.json` は後続作業でNoiseチャンネルへ変更する。
