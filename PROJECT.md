...
- 段階実装では、`SELECT LEVEL`、`EASY`、`NORMAL`、`HARD` の表示、`▶` カーソル表示、上下キーの選択変更、AボタンまたはSTARTボタンでのゲーム開始までを先に実装し、Bボタンでのタイトル復帰は後続WBSで実装する。
- 可変盤面サイズ対応が未完了のため、現時点では難易度選択後も盤面サイズ・地雷数は既存仕様のままとする。選択中難易度はWRAMに保持し、後続の可変盤面サイズ対応で参照する。
