...
- [ ] サウンド
  - [ ] サウンド基盤
    ...
    - [ ] MIDI素材からGame Boy用楽曲JSONを生成する運用を設計する
      ...
      - [ ] Waveチャンネル用InstrumentとWave tableの扱いを決める
      - [ ] Noiseチャンネル用Instrumentとノイズ周波数の扱いを決める
      - [ ] ループ開始・終了位置の指定方法を決める
      - [ ] 変換結果を人が音符単位で修正しなくても、試聴結果をもとにChatGPTへ自然言語で再調整を依頼できる運用を決める
    - [ ] 楽曲定義JSON仕様をMIDI変換対応へ拡張する
      - [ ] 現行JSON仕様でMIDI情報を保持できない項目を洗い出す
      ...
      - [ ] 拡張後のJSONサンプルを作成する
      - [ ] PROJECT.mdの「楽曲定義JSON仕様」を更新する
    - [ ] JSON変換ツールを拡張JSON仕様へ対応させる
      ...
      - [ ] `tools/json_to_uge.py` を拡張JSON仕様へ対応させる
      - [ ] 既存JSONを入力した場合の互換動作を確認する
      - [ ] hUGETracker Export ASMと比較し、再生に影響する差分がないことを確認する
      - [ ] `tools/README.md` に拡張JSON仕様と使用方法を追記する
