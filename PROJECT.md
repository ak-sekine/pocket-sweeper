...
- Noise効果音では、Noise用Instrument詳細または今後追加するNoise用JSON項目からNR41/NR42/NR43/NR44を生成する。
- `priority` は効果音JSONへ追加する想定とする。未指定時のデフォルト優先度は効果音種別ごとに `json_to_sfx_asm.py` 側で定義する。
...
- SFX用APU書き込みデータの具体的なASMフォーマットは実装前に決める。
...
- [ ] サウンド
  ...
  - [ ] サウンド実装
    ...
    - [ ] 効果音JSONへ本番SFX用priority項目を追加する
    - [ ] Noise効果音用JSON詳細項目を決める
    - [ ] JSONから効果音用SFX ASMを生成するPythonツールを作成する
      - [ ] `tools/json_to_sfx_asm.py` を作成する
      - [ ] Pulse用APUレジスタ書き込みデータを出力する
      - [ ] Noise用APUレジスタ書き込みデータを出力する
