...
- リポジトリ名、Makefileの `PROJECT` 値、`obj/` / `build/` 配下の `gb-minesweeper.*` 生成物名は、影響範囲確認が必要なため現時点では維持する。
...
- [ ] 検証・公開準備
  ...
  - [ ] タイトル変更に伴うプロジェクト名・生成物名の統一
    - [ ] リポジトリ名を `gb-minesweeper` から `pocket-sweeper` へ変更する
    - [ ] Makefileの `PROJECT` 値を `pocket-sweeper` へ変更する
    - [ ] ROM成果物名を `build/pocket-sweeper.gb` へ変更する
    - [ ] obj配下の `.sym` / `.map` などの生成物名を `pocket-sweeper.*` へ変更する
    - [ ] README内の旧タイトル・旧成果物名を更新する
    - [ ] ソース・Makefile・README・PROJECT.md全体から `gb-minesweeper` / `GB Minesweeper` / `gb-minesweeper.gb` の残存を検索し、必要な箇所を修正する
    - [ ] `make clean && make` で `build/pocket-sweeper.gb` が生成されることを確認する
