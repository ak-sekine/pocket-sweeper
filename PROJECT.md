...
- SFX開始時はchannel kindに応じて `hUGE_mute_channel` で対象チャンネルをミュートする。Pulse1効果音はCH1、Noise効果音はCH4を使う。
...
- 操作別のSFX呼び出しは後続WBSで行う。現時点では、カーソル移動などのゲーム入力から `Sound_PlaySfx` を呼び出す処理は未実装である。
