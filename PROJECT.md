...
- 将来的にはJSONからhUGEDriver用RGBDS ASMを直接生成するツールを作る。
- さらにASMからサウンド再生確認用テストROMを生成するPythonツールを作る。
...
- [ ] サウンド
  ...
  - [ ] JSONからhUGEDriver用RGBDS ASMを直接生成するPythonツールを作成する
    - [ ] `tools/json_to_huge_asm.py` を作成する
    - [ ] JSONを読み込む
    - [ ] note / length / instrument / effect をASMへ変換する
    - [ ] order / pattern / instrument / routine / wave のASMを出力する
    - [ ] wave/noise未使用時の空patternを出力する
    - [ ] サンプルJSONからASMを生成する
    - [ ] hUGETracker Export ASMと比較する
    - [ ] `tools/README.md` に使い方を追記する
  - [ ] サウンド再生確認用テストROMを生成するPythonツールを作成する
    - [ ] `tools/build_sound_test_rom.py` を作成する
    - [ ] 指定したASMをincludeする最小ROM用ASMを生成する
    - [ ] hUGEDriverを組み込む
    - [ ] 起動後に指定曲を再生する
    - [ ] `rgbasm` / `rgblink` / `rgbfix` を呼び出してGB ROMを生成する
    - [ ] `obj/` に中間ファイルを出力する
    - [ ] `build/` にテストROMを出力する
    - [ ] `tools/README.md` に使い方を追記する
  ...
  - [ ] BGM
    ...
    - [ ] JSON修正と再生成でBGMを調整する
    - [ ] BGMのASM生成とROM組み込みを行う
  - [ ] 効果音
    - [ ] 楽曲定義JSONから効果音下書きを作成する
    - [ ] JSON修正と再生成で効果音を調整する
    - [ ] 効果音のASM生成とROM組み込みを行う
- [ ] 検証・公開準備
  - [ ] テスト
