...
class WorkflowCheckAssetTests(unittest.TestCase):
    ...
    def test_workflow_check_metadata_orders_and_patterns_match_sound_spec(self):
        ...
        self.assertEqual(
            ...
            {"version": 2, "type": "bgm", "title": "Version 2 BGM Workflow Check", "tempo": 5},
        )
    ...
    def test_workflow_check_note_ranges_lengths_volumes_and_noise_mapping(self):
        ...
        for name, expected_hits in expected_noise.items():
            ...
            for c in cells:
                if c['note'] != 'rest': hits.append((row,c['instrument'],c['note']))
                self.assertNotIn('volume', c) if c['note']=='rest' else None
                row += c['length']
