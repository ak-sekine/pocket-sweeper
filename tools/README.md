...
python tools/build_sound_test_rom.py --ch4-mute-toggle obj/bgm_v2_ch4_mute_audibility_test.asm build/bgm_v2_ch4_mute_audibility_test.gb
```

SameBoyで起動し、次の順序で確認する。SelectボタンをCH4 SOLOの切り替えに使う。

1. `ALL CHANNELS` で通常再生を確認する。
2. Selectボタンで `CH4 SOLO` に切り替える。
3. CH4単独のNoiseが明確に聞こえることを確認する。
4. Bボタンで `ALL CHANNELS` に戻す。
5. Aボタンで `CH4 MUTED` にする。
6. SOLOで確認したNoiseだけが消えることを確認する。
7. CH1～CH3で主旋律、調性、テンポ、拍、フレーズ、ループ境界が維持されることを確認する。
8. Bボタンで通常再生へ戻す。
9. CH4が現在位置から自然に復帰し、遅延Noiseがないことを確認する。

CH4 SOLOでもNoiseが聞こえない場合は、このWBSを完了せず、CH4発音経路の不具合として報告する。A/B操作、方向キーの無操作、押下エッジ検出の既存仕様は維持する。

## CH2・CH4同時ミュート確認用ROM

CH1を主旋律、CH3をベースとする4チャンネル曲で、再生中のdriver muteによりCH2とCH4だけを同時に消音する。

```bash
python tools/json_to_huge_asm.py assets/bgm_v2_ch1_ch3_skeleton_test.json obj/bgm_v2_ch1_ch3_skeleton_test.asm
python tools/build_sound_test_rom.py --ch2-ch4-mute-toggle obj/bgm_v2_ch1_ch3_skeleton_test.asm build/bgm_v2_ch1_ch3_skeleton_test_ch2_ch4_mute.gb
```

SameBoyで対象ROMを起動し、次を確認する。

1. `ALL CHANNELS` でCH1〜CH4を再生する。
2. Selectで `CH1 CH3 SOLO` に切り替え、CH1の主旋律とCH3のベース／持続音を聴き分ける。
3. Bで `ALL CHANNELS` に戻し、Aで `CH2 CH4 MUTED` にする。
4. CH2の補助旋律／伴奏とCH4のNoiseリズムだけが消え、CH1/CH3の曲として認識できる主旋律、調性・和声の基準、テンポ、拍、フレーズ進行、ループ境界が維持されることを確認する。
5. 無音、異常音、テンポ停止、位置の巻き戻りがなく、`ALL CHANNELS` へ戻すとCH2/CH4が現在位置から自然に復帰することを確認する。過去のnote／Noiseが遅れてまとめて発音されてはならない。
6. 状態切り替えを複数回繰り返す。方向キーでは状態が変わらず、ボタン長押しでは連続切り替えされないことも確認する。

次の場合は不合格とする：CH1/CH3だけで主旋律や進行を認識できない、CH2/CH4の音が残る、ミュート中にテンポ・ループ位置がずれる、復帰時に過去の発音が遅れて鳴る、異常音・意図しない無音・停止が起きる、またはSOLOと同時ミュートの音が一致しない。人がこの確認を実施するまでWBSを完了にしない。
